...
% Definir la funciÃ³n de la curva
...
area = 0;
for i = 1:12
   area = area + pesos(i) * f(puntos(i)); %Revisar el intervalo transformacion lineal
end
...
function puntos = gauss_legendre(n)
    % Esta funcion devuelve los puntos de Gauss-Legendre para n puntos.
    puntos = zeros(n, 1); % Inicializar el vector de puntos
    for k = 1:n
        % Inicializar una aproximacion inicial
        x0 = cos(pi * (k - 0.25) / (n + 0.5));
        % MÃ©todo de Newton-Raphson
        for j = 1:100 % Iteraciones maximas
            P0 = 1;
            P1 = x0;
            for i = 2:n
            ...
            if abs(dP) < 1e-10 % Evitar division por cero
                break;
            end
...
function pesos = gauss_legendre_peso(n)
    % Esta funciÃ³n devuelve los pesos de Gauss-Legendre para n puntos.
